...
- **Request Body**:
  ...
  export default class extends Controller {
    static targets = ["form", "name", "email", "question", "context", "success", "error"];
    ...
    async submit(event) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          this.errorTarget.textContent = data.message || "Failed to submit question";
          this.errorTarget.classList.remove("hidden");
    ...
    async loadPendingQuestions() {
      try {
        ...
        data.questions.forEach((question) => {
          ...
          clone.querySelector(".question-context").textContent = question.context || "No context provided";
          clone.querySelector(".question-timestamp").textContent = new Date(question.timestamp).toLocaleString();
          clone.querySelector(".question-link").href = `/responder/conversation.html?id=${question.id}`;
  ...
  export default class extends Controller {
    static targets = ["questionText", "questionContext", "messagesList", "messageForm", "messageInput", "emptyState"];
    ...
    async loadQuestion() {
      try {
        ...
        this.questionContextTarget.textContent = question.context || "No context provided";
      } catch (error) {
    ...
    renderMessages(messages) {
      ...
      messages.forEach((message) => {
        ...
        messageElement.classList.add("message", message.sender === "responder" ? "message-responder" : "message-user");
    ...
    async closeConversation() {
      ...
      try {
        ...
        const response = await fetch(`/api/conversations/${this.conversationId}/close`, {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  export default class extends Controller {
    static targets = ["articlesList", "articleTemplate", "searchInput", "categoryFilter"];
    ...
    async loadArticles() {
      try {
        ...
        data.articles.forEach((article) => {
          ...
          clone.querySelector(".article-link").href = `/knowledge-base.html?id=${article.id}`;
  ...
  export default class extends Controller {
    static targets = ["form", "conversationId", "includeContext", "format", "summary", "keyPoints", "error"];
  ...
  <div data-controller="authentication">
    <form data-action="submit->authentication#login" data-authentication-target="form">
      <div class="mb-4">
      ...
      <div data-authentication-target="error" class="text-red-500 mb-4 hidden"></div>
      <button
        type="submit"
    ...
    <template data-dashboard-target="questionTemplate">
      <div class="bg-white shadow rounded-lg p-4">
        <div class="flex justify-between items-start">
          <div>
            <p class="text-sm text-gray-500">Question ID: <span class="question-id"></span></p>
  ...
  <div data-controller="conversation">
    ...
    <div class="bg-white shadow rounded-lg p-4 mb-4">
      ...
      <div data-conversation-target="messagesList" class="space-y-4 max-h-96 overflow-y-auto mb-4"></div>

      <div data-conversation-target="emptyState" class="hidden">
        <p class="text-gray-500 mb-4">No messages yet. Start the conversation by sending a response.</p>
        <form data-action="submit->conversation#startConversation">
          <div class="mb-4">
            <label for="initialResponse" class="block text-sm font-medium text-gray-700">Initial Response</label>
            <textarea
              id="initialResponse"
      ...
      <form data-conversation-target="messageForm" data-action="submit->conversation#sendMessage" class="hidden">
        <div class="mb-4">
      ...
      <div class="bg-white shadow rounded-lg p-4">
        <div class="flex justify-between items-start">
          <div>
            ...
            <p class="text-sm text-gray-500 mt-2">Category: <span class="article-category"></span></p>
            <div class="article-tags flex flex-wrap gap-2 mt-2"></div>
          </div>
        </div>
        <div class="mt-4">
          <a href="#" class="article-link text-indigo-600 hover:text-indigo-800"> Read more </a>
        </div>
      </div>
    </template>
  </div>
  ...
  <div data-controller="summarization">
    ...
    >
      <div class="mb-4">
        <label for="conversationId" class="block text-sm font-medium text-gray-700">Conversation ID</label>
        <input
          type="text"
          id="conversationId"
      ...
      <div class="mb-4 flex items-center">
        ...
        />
        <label for="includeContext" class="ml-2 block text-sm text-gray-700">Include Context</label>
      </div>
      <div class="mb-4">
      ...
      <div data-summarization-target="error" class="text-red-500 mb-4 hidden"></div>
      <button
        type="submit"
        class="inline-flex justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
      >
    ...
    <div class="bg-white shadow rounded-lg p-4">
      ...
      <ul data-summarization-target="keyPoints" class="list-disc pl-5 text-gray-600"></ul>
    </div>
  </div>
  ```
